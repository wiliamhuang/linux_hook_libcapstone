...
determine_lib_path(void)
{
	...
	if ((end - start + 1) > sizeof(path_ld)) {
		printf("Warning: path of ld.so is too long.\n");
		goto err;
	}
	...
	/* compose path_libpthread based on path_libc string */
	...
	memcpy(pos + 3 + 7, path_libc + (pos - path_libpthread + 4), end - start + 1 + 6 - (pos - path_libpthread + 4));
	if (path_libpthread[end - start + 1 + 4] >= '5' && path_libpthread[end - start + 1 + 4] <= '7') {
